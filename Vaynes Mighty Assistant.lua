...
function OnLoad()
    ...
    if not VIP_USER then
        VayneAssistant.settingsSubMenu:addParam("shootingMode", "Currently: No prediction", SCRIPT_PARAM_INFO, "NOT VIP")
    else
...
function OnDraw()
    ...
    if IsKeyDown(AllClassMenu) then
        ...
        VayneAssistant.settingsSubMenu._param[5].text  = VayneAssistant.settingsSubMenu.shootingMode and VIP_USER and "Currently: Using Predictions" or "Currently: No prediction"
        if not VIP_USER then VayneAssistant.settingsSubMenu.shootingMode = "NOT VIP" end
    end
            ...
            for i, enemyHero in ipairs(enemyTable) do
                if not VayneAssistant.condemnSubMenu["disableCondemn"..i] then
                    if enemyHero ~= nil and enemyHero.valid and not enemyHero.dead and enemyHero.visible and GetDistance(enemyHero) <= 715 and GetDistance(enemyHero) > 0 then
                        local enemyPosition = VayneAssistant.settingsSubMenu.shootingMode and VIP_USER and tp:GetPrediction(enemyHero) or enemyHero
